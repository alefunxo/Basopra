...
def _to_numeric_safe(series):
    '''Like pd.to_numeric(series, errors='ignore'), which pandas deprecated:
    convert if possible, otherwise leave the column untouched.'''
    try:
...
def get_base_prices(country,App_comb,PV_nom,df_base,df_batt,param):
    ...
    df_base.loc[df_base.rem_load<0,'rem_load']=0
    df_base.loc[df_base.surplus<0,'surplus']=0
    df_base['curt']=df_base.E_PV*0
    bill_power=0
    bill_power_PV=0
    bill_power_batt=0
    if App_comb[0]:
        df_base.loc[df_base.surplus>PV_nom*curtailment*dt,'curt']=df_base.surplus[df_base.surplus>PV_nom*curtailment*dt]-PV_nom*curtailment*dt
        df_base.loc[df_base.curt<0,'curt']=0
    if App_comb[3]:
...
def get_main_results(dict_res,param):
    ...
    logger.debug('out of agg_results')
    return[agg_results]
